...
0. Learn the logic flow form the measurement example feature folder.
...
3. In pages/tabs that display important data related to user (especially home tabs):
   - Add listener to GlobalRefresher in initState.
   - when the page/tab loads trigger load().
   - After mutation -> When event received → call .refresh() on the relevant cubit
...
7. document what have changed in a /features/user/change_log.md file

Start with the most common mutation flows (meal logging and glucose entry are good candidates).

## Note: DON'T START WITHOUT A PLAN AND A SPECIFIED TASKS.
```
